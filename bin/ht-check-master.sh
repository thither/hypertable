...
usage() {
  echo
  echo "usage: ht-check-master.sh [OPTIONS] [<server-options>]"
  echo
  echo "OPTIONS:"
  ...
  echo
  echo "Checks the status of the Master process.  This script is Nagios plugin"
  ...
  echo "format:"
  echo
  echo "  Master <status> - <description>"
  echo
}
...
$RUNTIME_ROOT/bin/ht master ${TIMEOUT_OPTION} --batch -e "status" "$@" ${SERVICE_HOSTNAME}
