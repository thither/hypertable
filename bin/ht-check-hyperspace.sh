...
usage() {
  echo
  echo "usage: ht-check-hyperspace.sh [OPTIONS] [<server-options>]"
  echo
  echo "OPTIONS:"
  ...
  echo
  echo "Checks the status of the Hyperspace process.  This script is Nagios plugin"
  ...
  echo "format:"
  echo
  echo "  Hyperspace <status> - <description>"
  echo
}
...
$RUNTIME_ROOT/bin/ht hyperspace ${TIMEOUT_OPTION} --batch -e "status" "$@" ${SERVICE_HOSTNAME}
