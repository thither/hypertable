...
public class RequestHandlerRmdir extends ApplicationHandler {

  static final Logger log = Logger.getLogger("org.hypertable");

  static final byte VERSION = 1;

  public RequestHandlerRmdir(Comm comm, Broker broker, Event event) {
    super(event);
    mComm = comm;
    mBroker = broker;
  }
  ...
  public void run() {
    ...
    try {
      ...
      if (version != VERSION)
        throw new ProtocolException("Rmdir parameters version mismatch, expected " +
                                    VERSION + ", got " + version);
