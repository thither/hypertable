...
/**
 */
void CellStoreTrailerV3::serialize(uint8_t *buf) {
  uint8_t *base = buf;
  ...
  assert(version == 3);
  assert((buf-base) == (int)CellStoreTrailerV3::size());
  (void)base;
}
...
/**
 */
void CellStoreTrailerV3::deserialize(const uint8_t *buf) {
  HT_TRY("deserializing cellstore trailer",
    size_t remaining = CellStoreTrailerV3::size();
...
/**
 */
void CellStoreTrailerV3::display(std::ostream &os) {
  os << "{CellStoreTrailerV3: ";
...
/**
 */
void CellStoreTrailerV3::display_multiline(std::ostream &os) {
  os << "[CellStoreTrailerV3]\n";
  os << "  fix_index_offset: " << fix_index_offset << "\n";
  os << "  var_index_offset: " << var_index_offset << "\n";
  os << "  filter_offset: " << filter_offset << "\n";
