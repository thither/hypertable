...
/// @file
/// Definitions for Seek request handler.
/// This file contains definitions for Seek, a server-side request handler
...
void Seek::run() {
  ...
  try {
    Request::Parameters::Seek params;
    params.decode(&ptr, &remain);
    m_broker->seek(&cb, params.get_fd(), params.get_offset());
  }
  catch (Exception &e) {
