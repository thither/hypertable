...
/// @file
/// Definitions for Close request handler.
/// This file contains definitions for Close, a server-side request handler
...
void Close::run() {
  ...
  try {
    Request::Parameters::Close params;
    params.decode(&ptr, &remain);
    m_broker->close(&cb, params.get_fd());
  }
  catch (Exception &e) {
