 ...
 *
 */
void RequestHandlerAttrIncr::run() {
  ResponseCallbackAttrIncr cb(m_comm, m_event);
  ...
  try {
    ...
    m_master->attr_incr(&cb, m_session_id, handle, name, attr);
  }
  catch (Exception &e) {
    HT_ERROR_OUT << e << HT_END;
    cb.error(e.code(), "Error handling ATTRINCR message");
  }
}
