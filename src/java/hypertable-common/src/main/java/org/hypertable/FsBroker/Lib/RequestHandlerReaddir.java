...
public class RequestHandlerReaddir extends ApplicationHandler {
  ...
  public RequestHandlerReaddir(Comm comm, Broker broker, Event event) {
    super(event);
    mComm = comm;
    mBroker = broker;
  }
  ...
  public void run() {
    String  fileName;
    ResponseCallbackReaddir cb = new ResponseCallbackReaddir(mComm, mEvent);
    ...
    try {
      ...
      if (version != VERSION)
        throw new ProtocolException("Readdir parameters version mismatch, expected " +
                                    VERSION + ", got " + version);
