...
public class RequestHandlerRename extends ApplicationHandler {
  ...
  public RequestHandlerRename(Comm comm, Broker broker, Event event) {
    super(event);
    mComm = comm;
    mBroker = broker;
  }
  ...
  public void run() {
    ...
    try {
      ...
      if (version != VERSION)
        throw new ProtocolException("Rename parameters version mismatch, expected " +
                                    VERSION + ", got " + version);
      ...
      String src, dst;
      if ((src = Serialization.DecodeVStr(mEvent.payload)) == null ||
          (dst = Serialization.DecodeVStr(mEvent.payload)) == null)
